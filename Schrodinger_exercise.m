...
coeff = -h_bar^2/(2*mass);
for i=2:(N-1)
  ham(i,i-1) = coeff;
  ham(i,i) = -2*coeff; %plus V(i) % Set interior rows
  ham(i,i+1) = coeff;
end
...
elseif(imethod==2) % Implicit FTCS
    A = inv(eye(N) + i_imag*tau/h_bar*ham);
else               % Crank-Nicolson
    A = inv(eye(N) + i_imag*tau/(2*h_bar)*ham)*(eye(N) - i_imag*tau/(2*h_bar)*ham);
end
...
p0 = mass*velocity;
k0 = p0/h_bar;       % Average wavenumber
